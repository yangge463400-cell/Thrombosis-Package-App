<template>
  <div>
    <div class="list-head">
      <div class="page-title">医院管理员</div>
      <el-button type="primary" @click="openDialog()">＋ 新增医院管理员</el-button>
    </div>

    <div class="filter-bar">
      <el-select v-model="filterHospitalId" placeholder="全部医院" clearable style="width:220px;" @change="load">
    ...
    <el-table :data="list" stripe style="margin-top:16px;">
      ...
      <el-table-column prop="phone" label="登录账号" width="160" />
      <el-table-column prop="nickname" label="姓名" min-width="140" />
      <el-table-column label="所属医院" min-width="220">
    ...
    <!-- 新增/编辑医院管理员 -->
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑医院管理员' : '新增医院管理员'" width="480px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="所属医院" required>
          <el-select v-model="form.hospitalId" placeholder="请选择医院（如 复旦大学附属中山医院）" style="width:100%;">
            <el-option v-for="h in hospitals" :key="h.id" :label="h.name" :value="h.id" />
          </el-select>
        </el-form-item>
...
async function load() {
  try {
    ...
    const data = await adminApi.hospitalAdmins(params);
...
async function onSave() {
  ...
  if (!form.value.hospitalId) { ElMessage.warning('请选择所属医院'); return; }
  saving.value = true;
  ...
  try {
    if (form.value.id) {
      await adminApi.updateHospitalAdmin(form.value.id, payload);
    } else {
      await adminApi.createHospitalAdmin(payload);
    }
...
async function onDelete(row) {
  try {
    await ElMessageBox.confirm(`确定删除医院管理员「${row.nickname || row.phone}」吗？删除后该账号将无法登录医院端。`, '删除确认', { type: 'warning' });
    await adminApi.deleteHospitalAdmin(row.id);
...
async function onResetSave() {
  ...
  saving.value = true;
  try {
    await adminApi.resetHospitalAdminPassword(resetTarget.value.id, resetPassword.value);
